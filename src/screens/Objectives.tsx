...
const LEVEL_META: Record<Level, { label: string; color: string; icon: React.ElementType }> = {
  COMPANY:    { label: 'Company',    color: '#A855F7', icon: Building2 },
  MANAGER:    { label: 'Manager',    color: '#3B82F6', icon: Users },
  INDIVIDUAL: { label: 'Individual', color: '#22C55E', icon: UserIcon },
}
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: '6px', padding: '7px 10px', fontSize: '12px', color: 'var(--text-primary)', outline: 'none', boxSizing: 'border-box',
}
...
export function Objectives({ userId, userRole }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '17px' }}>Objectives</h1>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <select value={quarter} onChange={e => setQuarter(Number(e.target.value))} style={{ ...inputStyle, width: 'auto' }}>
              {[1, 2, 3, 4].map(q => <option key={q} value={q}>Q{q}</option>)}
            </select>
            <select value={year} onChange={e => setYear(Number(e.target.value))} style={{ ...inputStyle, width: 'auto' }}>
              {[now.getFullYear() - 1, now.getFullYear(), now.getFullYear() + 1].map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <button onClick={() => setCreating(true)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 12px', background: 'var(--accent)', color: '#000', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 600, whiteSpace: 'nowrap' }}>
              <Plus size={14} /> New
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '14px', overflowX: 'auto' }}>
          {[
            { label: 'Company', value: overall(company), color: '#A855F7' },
            { label: 'Mine', value: overall(mine), color: '#22C55E' },
            { label: 'Objectives', value: objectives.length, color: 'var(--text-primary)', raw: true },
          ].map(s => (
            <div key={s.label} style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: '8px', padding: '8px 14px', flexShrink: 0, minWidth: '90px' }}>
              <div style={{ color: s.color, fontFamily: 'var(--font-mono)', fontSize: '18px', fontWeight: 700, lineHeight: 1 }}>{s.value}{s.raw ? '' : '%'}</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <Section title="Company Objectives" objs={company} />
        <Section title="My Objectives" objs={mine} />
        {(canCreateForOthers || team.length > 0) && <Section title="Team Objectives" objs={team} />}
      </div>
  ...
  function Section({ title, objs }: { title: string; objs: Objective[] }) {
  ...
  function ObjectiveCard({ obj }: { obj: Objective }) {
    ...
    const objMonths = months.filter(m => m.objective_id === obj.id).sort((a, b) => a.month - b.month)
    const myEval = evals.find(e => e.objective_id === obj.id && e.user_id === userId)
    const editable = canEdit(obj)
    ...
    return (
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: '10px', padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>{obj.title}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '9px', fontFamily: 'var(--font-mono)', color: meta.color, background: `${meta.color}18`, border: `1px solid ${meta.color}40`, borderRadius: '4px', padding: '1px 6px' }}>
          ...
          {editable && (
            <button onClick={() => deleteObjective(obj.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-tertiary)', padding: '2px' }} title="Delete"><Trash2 size={13} /></button>
          )}
        </div>
        ...
        <div style={{ marginTop: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
            ...
            <span style={{ fontSize: '13px', fontWeight: 700, fontFamily: 'var(--font-mono)', color: progress >= 100 ? '#22C55E' : 'var(--accent)' }}>{progress}%</span>
          </div>
          <div style={{ height: '8px', background: 'var(--bg-base)', borderRadius: '4px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress}%`, background: progress >= 100 ? '#22C55E' : 'var(--accent)', transition: 'width 0.3s' }} />
          </div>
          {editable && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '8px' }}>
              ...
              <input
                ...
                onBlur={e => { const v = Number(e.target.value); if (v !== obj.current_value) updateCurrent(obj, v) }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '10px' }}>
          <button onClick={() => toggleExpand(obj)} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: '11px' }}>
            {isOpen ? <ChevronDown size={13} /> : <ChevronRight size={13} />} Desglose mensual
          </button>
          {isOwner && (
            <button onClick={() => setEvalOpen(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: myEval ? '#EAB308' : 'var(--text-secondary)', fontSize: '11px' }}>
              <Star size={13} style={{ fill: myEval ? '#EAB308' : 'none' }} /> {myEval ? `Autoeval: ${myEval.score}/5` : 'Autoevaluar'}
            </button>
          )}
        </div>
        ...
        {/* Monthly breakdown */}
        {isOpen && (
          <div style={{ marginTop: '10px', padding: '10px', background: 'var(--bg-base)', borderRadius: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {objMonths.map(m => {
              const mp = pct(m.current_value, m.target_value)
              return (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ width: '32px', fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>{MONTHS[m.month - 1]}</span>
                  <div style={{ flex: 1, height: '6px', background: 'var(--bg-surface)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${mp}%`, background: 'var(--accent)' }} />
                  </div>
                  {editable ? (
                    <>
                      <input type="number" defaultValue={m.current_value} onBlur={e => updateMonth(m, 'current_value', Number(e.target.value))} style={{ ...inputStyle, width: '60px', padding: '3px 6px', fontSize: '11px' }} title="Actual" />
                      <span style={{ fontSize: '10px', color: 'var(--text-tertiary)' }}>/</span>
                      <input type="number" defaultValue={m.target_value} onBlur={e => updateMonth(m, 'target_value', Number(e.target.value))} style={{ ...inputStyle, width: '60px', padding: '3px 6px', fontSize: '11px' }} title="Meta" />
                    </>
                  ) : (
                    <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary)' }}>{m.current_value}/{m.target_value}</span>
                  )}
                </div>
              )
            })}
        ...
        {evalOpen && isOwner && (
          <div style={{ marginTop: '10px', padding: '10px', background: 'var(--bg-base)', borderRadius: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              ...
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} onClick={() => setScore(n)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                  <Star size={16} style={{ color: '#EAB308', fill: n <= score ? '#EAB308' : 'none' }} />
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '8px' }}>
              <button onClick={() => setEvalOpen(false)} style={{ padding: '5px 12px', background: 'transparent', border: '1px solid var(--border-subtle)', color: 'var(--text-secondary)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>Cancel</button>
              <button onClick={() => { if (score) { saveSelfEval(obj, score, note); setEvalOpen(false); setNote('') } }} disabled={!score} style={{ padding: '5px 12px', background: 'var(--accent)', color: '#000', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 600, opacity: score ? 1 : 0.5 }}>
                <Check size={12} style={{ display: 'inline', marginRight: '4px' }} />Guardar
              </button>
            </div>
            {/* Past evals */}
            {evals.filter(e => e.objective_id === obj.id).map(e => (
              <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px', paddingTop: '8px', borderTop: '1px solid var(--border-subtle)' }}>
                ...
                <span style={{ fontSize: '11px', color: '#EAB308', fontFamily: 'var(--font-mono)' }}>{e.score}/5</span>
...
function CreateModal({ onClose, onCreated, year, quarter, userId, canCreateCompany, canCreateForOthers, teamMembers }: {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '12px', padding: '20px', width: '100%', maxWidth: '440px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
          <h2 style={{ color: 'var(--text-primary)', fontSize: '15px', fontWeight: 700 }}>New Objective — Q{quarter} {year}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '16px' }}>
          <button onClick={onClose} style={{ padding: '8px 14px', background: 'transparent', border: '1px solid var(--border-subtle)', color: 'var(--text-secondary)', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>Cancel</button>
          <button onClick={save} disabled={saving || !title.trim()} style={{ padding: '8px 16px', background: 'var(--accent)', color: '#000', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, opacity: saving || !title.trim() ? 0.5 : 1 }}>
